...
const TimeTrackerHeader = ({
  ...
  const getHeaderTitle = () => {
    ...
    switch (viewMode) {
      ...
      case "month":
        return format(selectedDate, "yyyy년", {
          locale: ko
        });
    }
  };
  return <header className="sticky top-0 z-30 bg-background/95 backdrop-blur-sm border-b border-border">
      ...
      {viewMode === "day" && <div className="flex justify-around px-2 py-2 border-t border-border">
          {weekDays.map((day, i) => {
        const isSelected = format(day, "yyyy-MM-dd") === format(selectedDate, "yyyy-MM-dd");
        return <button key={i} onClick={() => onDateSelect(day)} className="flex flex-col items-center gap-1">
                <span className="text-xs text-muted-foreground">
                  {format(day, "EEE", {
              locale: ko
            })[0]}
                </span>
                <motion.div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold ${isSelected ? "bg-primary text-primary-foreground" : "text-foreground"}`} whileTap={{
            scale: 0.95
          }}>
                  {format(day, "d")}
                </motion.div>
